...
| **สแกนด้วยกล้อง** | กดไอคอนกล้องท้ายช่องสแกน หรือ **F6** → เปิดหน้าต่างกล้อง เล็งบาร์โค้ดหรือ **คิวอาร์โค้ด** ในกรอบ ระบบเพิ่มสินค้าให้ทันทีพร้อมเสียง "ติ๊ด" แล้วสแกนชิ้นถัดไปได้เลย (ยกสินค้าชิ้นเดิมออกจากกรอบสักครู่ถ้าต้องการนับซ้ำ) กด **เสร็จสิ้น** เมื่อครบ |
...
**รหัสที่อ่านได้**: EAN-13, EAN-8, UPC-A, UPC-E, Code 128, Code 39, ITF, Codabar และ **QR Code** — ทั้งจากกล้องและจากเครื่องยิงที่รองรับรหัสนั้น ถ้าสินค้าติดป้าย QR ให้บันทึกข้อความในคิวอาร์นั้นเป็นบาร์โค้ดของสินค้า (สินค้า → แก้ไข → บาร์โค้ด) แล้วสแกนได้เหมือนบาร์โค้ดปกติ
...
> **ไปหน้าตั้งค่าจากหน้าขายได้เลย**: แถบเครื่องมือด้านบนของหน้าขายมีปุ่ม **ตั้งค่า** (เห็นเฉพาะผู้จัดการ/เจ้าของร้าน) กดแล้วไปหน้าตั้งค่าทั้งหมดได้ทันทีโดยไม่ต้องเปิดเมนูด้านซ้าย
...
เมนู **คลังสินค้า** หน้าแรกแสดง **จำนวนหน่วยคงเหลือ**, **มูลค่าต้นทุน**, **มูลค่าขาย** และทางลัดไปแต่ละงาน ทุกการเปลี่ยนแปลงสต็อกถูกบันทึกใน **ความเคลื่อนไหว** (ledger) แก้ตัวเลขคงเหลือตรงๆ ไม่ได้ ต้องผ่านเอกสารต่อไปนี้

### 7.1 เช็คสต็อกด้วยมือถือ (สแกนดูยอดคงเหลือ)

เมนู **เช็คสต็อก** (อยู่ในเมนูด้านซ้าย และในหน้าคลังสินค้า) ออกแบบมาให้ใช้บนมือถือขณะเดินดูของบนชั้น พนักงานทุกระดับใช้ดูยอดได้

1. เปิดเมนู **เช็คสต็อก** → กด **สแกนด้วยกล้อง** แล้วเล็งบาร์โค้ด/คิวอาร์โค้ดของสินค้า (หรือยิงด้วยเครื่องยิง หรือพิมพ์รหัสแล้วกด Enter)
2. ระบบแสดงทันที: **ยอดคงเหลือในระบบ** ตัวใหญ่พร้อมหน่วย, สถานะสต็อก (สต็อกปกติ / ใกล้หมด / ต้องสั่งด่วน), **ราคาขาย**, **ต้นทุนเฉลี่ย**, **มูลค่าคงเหลือ**, **จุดสั่งซื้อ** และปุ่มไปดู **ความเคลื่อนไหว** ของสินค้านั้น
3. สแกนชิ้นถัดไปได้เรื่อยๆ รายการที่สแกนไปแล้วอยู่ในกลุ่ม **สแกนล่าสุด** ด้านล่าง แตะเพื่อกลับไปดูซ้ำ

**บันทึกการนับไปด้วย** (ผู้จัดการ/เจ้าของร้าน): เปิดสวิตช์ **โหมดบันทึกการนับสต็อก**

- เลือกใบนับที่เปิดอยู่ หรือกด **เปิดใบนับใหม่** — ใบที่เปิดจากมือถือจะ **เริ่มจากใบเปล่า** แล้วเพิ่มเฉพาะสินค้าที่สแกน (ไม่ดึงสินค้าทั้งร้านมาใส่)
- หลังสแกนแต่ละชิ้น กรอก **จำนวนที่นับได้** ระบบบอกผลต่างจากยอดในระบบทันที (เกิน/ขาด) แล้วกด **บันทึกการนับ**
- เมื่อนับครบ เปิดใบนับที่หน้า **คลังสินค้า → ตรวจนับสต็อก** เพื่อดูสรุปและ **ปิดการตรวจนับ** (ข้อ 7.4) ระบบจะปรับสต็อกให้ตรงกับที่นับได้

**ปรับสต็อกให้ตรงทันที**: ถ้าของบนชั้นไม่ตรงกับระบบและไม่อยากเปิดใบนับ ให้เปิด **โหมดบันทึกการนับสต็อก** กรอกจำนวนที่นับได้ แล้วกด **ปรับสต็อกให้ตรง** ระบบจะสร้างเอกสารปรับปรุงสต็อก (`ADJ…`) เหตุผล "แก้ไขยอด" ให้เท่ากับส่วนต่างทันที และยอดบนหน้าจอจะอัปเดตให้เลย (ต้องเป็นผู้จัดการ/เจ้าของร้าน)

**แปลงหน่วยจากหน้าจอนี้**: ถ้าสินค้าที่สแกนมีสูตรแปลงหน่วย (ข้อ 7.5) จะมีช่อง **แปลงหน่วย** ให้กรอกจำนวนลังที่จะแตก ระบบบอกล่วงหน้าว่าจะได้กี่ขวด กดปุ่มแล้วตัดสต็อกลังและเพิ่มขวดให้ทันที

> กล้องมือถือต้องเปิดเว็บผ่าน **https** · ถ้าสแกนแล้วขึ้น "ไม่พบสินค้าสำหรับรหัส …" แปลว่าบาร์โค้ดนั้นยังไม่ได้ผูกกับสินค้าใด ให้เพิ่มบาร์โค้ดที่ **สินค้า → แก้ไข → บาร์โค้ด**

### 7.2 รับสินค้าเข้า (`OD…`)
...
### 7.5 แปลงหน่วยสินค้า (`CV…`) — ลัง → ขวด

ใช้เมื่อสินค้าชนิดเดียวกันขายทั้งหน่วยใหญ่และหน่วยย่อย เช่น เบียร์ขายทั้ง **ลัง** และ **ขวด** โดยแยกเป็นสินค้า 2 รายการ (คนละบาร์โค้ด คนละราคา) การแตกลังมาขายเป็นขวดต้องตัดสต็อกลังและเพิ่มสต็อกขวด **แปลงหน่วยสินค้า** ทำให้ครบทั้งสองด้านในครั้งเดียว

**คลังสินค้า → แปลงหน่วยสินค้า**

1. เลือก **สินค้าต้นทาง (หน่วยใหญ่)** เช่น เบียร์ (ลัง) และ **สินค้าปลายทาง (หน่วยย่อย)** เช่น เบียร์ (ขวด) — ระบบแสดงยอดคงเหลือของทั้งคู่
2. ใส่ **จำนวนต่อ 1 หน่วยใหญ่** (เช่น 12) ครั้งแรกครั้งเดียว แล้วกด **บันทึกเป็นสูตร** ครั้งต่อไประบบเติมให้อัตโนมัติ
3. ใส่ **จำนวนที่จะแตก** (เช่น 2 ลัง) ระบบแสดงตัวอย่างผลลัพธ์ "แตก 2 ลัง → ได้ 24 ขวด" แล้วกด **แปลงหน่วย**

ผลที่เกิดขึ้น (ตัวอย่าง: มี 5 ลัง ต้นทุนลังละ ฿480 แตก 2 ลัง)

| รายการ | ผล |
|---|---|
| สต็อกลัง | 5 → **3 ลัง** (บันทึกความเคลื่อนไหว `โอนออก` −2) |
| สต็อกขวด | 0 → **24 ขวด** (บันทึกความเคลื่อนไหว `โอนเข้า` +24) |
| ต้นทุน | ต้นทุนที่ตัดออก ฿960 ถูกโอนไปเป็นต้นทุนของขวด ฿40/ขวด (มูลค่าสต็อกรวมของร้านไม่เปลี่ยน) |
| เอกสาร | `CV6909-00001` เก็บไว้ในประวัติการแปลงหน่วย และในบันทึกการใช้งาน |

- แตกได้ไม่เกินจำนวนที่มีจริง (ถ้าไม่พอระบบจะไม่ให้กด)
- ปิดสูตรที่เลิกใช้ได้ที่ตาราง **สูตรการแปลงหน่วย** โดยไม่ลบประวัติ
- แปลงจากหน้า **เช็คสต็อก** บนมือถือได้ด้วย (ข้อ 7.1) เหมาะกับตอนยืนอยู่หน้าชั้นวาง
- ถ้าจะแปลงกลับ (เช่น รวมขวดกลับเป็นลัง) ให้สร้างสูตรอีกทิศทางหนึ่ง (ขวด → ลัง ใส่จำนวนต่อหน่วยเป็น 1/12 หรือใช้ใบปรับปรุงสต็อก)

### 7.6 ความเคลื่อนไหว

ตารางทุกรายการ: **วันเวลา**, สินค้า, **ประเภท** (ยอดยกมา / ขาย / ยกเลิกการขาย / รับคืน / รับสินค้าเข้า / ยกเลิกรับสินค้า / ปรับปรุง / ตรวจนับ / โอนออก-โอนเข้าจากการแปลงหน่วย), **เปลี่ยนแปลง**, **ต้นทุน/หน่วย**, **คงเหลือหลังรายการ**, เอกสารอ้างอิง, ผู้ทำรายการ กรองตามสินค้าและช่วงวันที่ได้
...
### 15.3 หน่วยนับสินค้า (ผู้จัดการขึ้นไป)

**ตั้งค่า → หน่วยนับสินค้า** จัดการหน่วยที่ใช้กับสินค้า (ขวด ห่อ ลัง กล่อง ซอง …)

- **เพิ่มหน่วยนับ**: ใส่ชื่อไทยและชื่อภาษาอังกฤษ (ไม่บังคับ) ชื่อซ้ำไม่ได้
- **แก้ไข**: กดไอคอนดินสอเพื่อเปลี่ยนชื่อ
- **ลบ = ปิดการใช้งาน**: สลับสวิตช์ **ใช้งาน** ออก ระบบจะถามยืนยัน หน่วยนั้นจะหายไปจากตัวเลือกตอนเพิ่ม/แก้ไขสินค้า แต่สินค้าและเอกสารเดิมที่ใช้หน่วยนี้ยังแสดงเหมือนเดิม (ข้อมูลเก่าไม่พัง) เปิดกลับมาใช้ใหม่ได้ทุกเมื่อ
- คอลัมน์ **สินค้าที่ใช้** บอกว่ามีสินค้ากี่รายการใช้หน่วยนั้นอยู่ ระบบจะเตือนก่อนปิดการใช้งานถ้ายังมีสินค้าใช้อยู่

### 15.4 ผู้ใช้งาน (เจ้าของร้าน)
...
### 15.7 ข้อมูลและการสำรอง (เจ้าของร้าน)

**ตั้งค่า → ข้อมูลและการสำรอง** รวมงานข้อมูลก้อนใหญ่ไว้ 3 อย่าง งานทั้งสามทำงานเบื้องหลัง ปิดหน้าจอไปทำอย่างอื่นแล้วกลับมาดูความคืบหน้าได้ และทำได้ทีละงานเท่านั้น

**1) สำรองข้อมูล** กด **สร้างไฟล์สำรองข้อมูล** ระบบจะรวมข้อมูลทั้งร้าน (สินค้า บาร์โค้ด สมาชิก หุ้น การขายทุกบิลพร้อมรายการ ลูกหนี้ การรับสินค้า สต็อก ค่าใช้จ่าย ปันผล ผู้ใช้งาน และการตั้งค่า) เป็นไฟล์ `.zip` เก็บไว้บนเซิร์ฟเวอร์ กด ⬇ เพื่อดาวน์โหลดเก็บไว้ที่อื่น (แนะนำให้ดาวน์โหลดเก็บนอกเซิร์ฟเวอร์อย่างน้อยเดือนละครั้ง) — ระบบจะส่งไฟล์ให้เบราว์เซอร์ดาวน์โหลดเอง ใช้ได้ทั้งคอมพิวเตอร์ แท็บเล็ต และ iPad โดยไฟล์จะไปอยู่ในแอป "ไฟล์"/โฟลเดอร์ดาวน์โหลดของเครื่อง (ลิงก์มีอายุ 5 นาที ถ้าค้างไว้นานแล้วกดไม่ติด ให้กดปุ่มดาวน์โหลดใหม่) ระบบเก็บไฟล์ล่าสุดไว้ 20 ไฟล์ ไฟล์เก่ากว่านั้นถูกลบอัตโนมัติ

- ข้อมูลตัวอย่างจากร้านจริง (สินค้า 6,614 · การขาย 231,710 บิล · รายการขาย 585,759) ได้ไฟล์ประมาณ 78 MB ใช้เวลาประมาณ 15 วินาที
- ไฟล์สำรอง **ไม่รวม** บันทึกการใช้งาน (audit) ประวัติผู้ช่วย AI และ session ที่ค้างอยู่

**2) กู้คืนข้อมูล** เลือกไฟล์สำรองบนเซิร์ฟเวอร์ หรืออัปโหลดไฟล์ `.zip` ที่เคยดาวน์โหลดไว้ แล้วเลือกตัวเลือก

| ตัวเลือก | ผล |
|---|---|
| **ล้างข้อมูลเดิมของร้านก่อนกู้คืน** (แนะนำ) | ลบข้อมูลปัจจุบันของร้านทั้งหมดแล้วใส่ข้อมูลจากไฟล์แทน — ได้สภาพเหมือนตอนสำรองเป๊ะ |
| ไม่ติ๊ก | เพิ่มเฉพาะรายการที่ยังไม่มี ข้อมูลเดิมที่ซ้ำจะไม่ถูกแก้ |
| **กู้คืนข้อมูลร้านด้วย** | นำชื่อร้าน ที่อยู่ เบอร์โทร ข้อความหัว/ท้ายใบเสร็จ จากไฟล์มาทับของเดิม |

ต้องติ๊กยืนยัน "ฉันเข้าใจและต้องการดำเนินการต่อ" ก่อนจึงกดกู้คืนได้ บัญชีผู้ใช้ที่กำลังทำรายการอยู่จะไม่ถูกลบ (กันไม่ให้ล็อกตัวเองออกจากระบบ) ข้อมูลขนาดร้านตัวอย่างใช้เวลาประมาณ 10 นาที ควรทำนอกเวลาขาย

**3) นำเข้าข้อมูลจากระบบเดิม (MS Access)** สำหรับร้านที่ย้ายมาจากโปรแกรม POS เดิม

1. ที่เครื่องคอมพิวเตอร์ที่มีไฟล์ `database.mdb` กดลิงก์ **ดาวน์โหลดสคริปต์ extract.ps1** จากหน้านี้ แล้วเปิด PowerShell ในโฟลเดอร์ที่เก็บสคริปต์ รัน `powershell -ExecutionPolicy Bypass -File extract.ps1 -Mdb D:/pos/database.mdb -Password (รหัสผ่านฐานข้อมูลเดิม) -Out dump -Zip` (สคริปต์เปิดไฟล์เดิมแบบอ่านอย่างเดียว ไม่แก้ไขข้อมูลเก่า)
2. ได้ไฟล์ `dump.zip` → กด **อัปโหลดไฟล์ข้อมูลระบบเดิม** หน้าจอจะแสดงจำนวนแถวของแต่ละตารางที่อ่านได้ ให้เทียบกับระบบเดิมคร่าวๆ
3. กด **ทดลองนำเข้า (ไม่บันทึก)** ระบบจะทำทุกขั้นตอนจริงแล้วยกเลิกทั้งหมดตอนจบ พร้อมรายงานต่อขั้นตอน (อ่าน/บันทึก/ข้าม/เวลา) และผล **ตรวจสอบยอดตรงกัน** 23 รายการ (จำนวนบิล ยอดขายรายปี จำนวนสมาชิก ทุนเรือนหุ้น ฯลฯ)
4. ถ้ายอดตรง กด **นำเข้าจริง** — นำเข้าซ้ำได้อย่างปลอดภัย รายการที่นำเข้าไปแล้วจะถูกข้าม (ดูรายละเอียดการแปลงข้อมูลใน `docs/MIGRATION.md`)

> ต้องเป็น **เจ้าของร้าน** จึงจะเห็นเมนูนี้ · ระบบล็อกไม่ให้ทำสองงานพร้อมกัน · ถ้ามีร้านหลายสาขา ให้เข้าสู่ระบบด้วยรหัสร้านของสาขานั้นแล้วนำเข้าไฟล์ของสาขานั้น
...
เมนู **ผู้ช่วย AI** (ผู้จัดการขึ้นไป) ใช้ถามข้อมูลร้านด้วยภาษาพูด — **เปิดใช้งานแล้วบนเซิร์ฟเวอร์ทดสอบ**

**วิธีใช้**

1. เปิดเมนู **ผู้ช่วย AI** พิมพ์คำถาม หรือกดตัวอย่างคำถามที่มีให้ (ยอดขายเดือนนี้ / สินค้าขายดี 10 อันดับ / ลูกหนี้ค้างชำระ / สินค้าสต็อกติดลบ) แล้วกด **ถาม** (หรือกด Enter)
2. ระบบตอบเป็น 3 ส่วน: **คำอธิบายเป็นภาษาไทย**, **ตารางผลลัพธ์**, และ **คำสั่งที่ระบบใช้ค้นข้อมูล** (กดคัดลอกไปตรวจสอบเองได้) พร้อมบอกจำนวนแถวและเวลาที่ใช้
3. คำถามล่าสุดอยู่ด้านล่าง แตะเพื่อถามซ้ำหรือแก้ข้อความ

ตัวอย่างที่ทดสอบแล้วบนข้อมูลจริง: "วันนี้ขายได้กี่บิล และยอดขายรวมเท่าไร" → ตอบ 3 บิล 213.00 บาท พร้อมคำสั่งที่ใช้ (ใช้เวลารวมประมาณ 2-6 วินาทีต่อคำถาม)

**ขอบเขตและความปลอดภัย**

- คำสั่งที่สร้างต้องเป็น **SELECT เท่านั้น** ระบบตรวจก่อนรันทุกครั้ง (แก้ไข/ลบข้อมูลไม่ได้เลย)
- เห็นได้เฉพาะข้อมูล **ร้านของคุณ** (บังคับด้วย row-level security เหมือนทุกหน้าจอ) และไม่เห็นตารางผู้ใช้/รหัสผ่าน
- จำกัดผลลัพธ์ 200 แถว และมีเวลาจำกัดต่อคำสั่ง ถ้าคำสั่งผิดระบบจะลองใหม่ให้หนึ่งครั้งพร้อมคำใบ้จากข้อผิดพลาด
- ทุกคำถามถูกบันทึกไว้ (คำถาม คำสั่งที่ใช้ จำนวนแถว เวลา) สำหรับตรวจสอบย้อนหลัง
- ถ้าตัวเลขสำคัญ ให้ตรวจกับ **รายงาน** มาตรฐานอีกครั้งเสมอ — AI ช่วยหาคำตอบเร็ว แต่รายงานคือตัวเลขอ้างอิง

ถ้าเมนูขึ้นว่า "ยังไม่เปิดใช้งาน" หรือ "ติดต่อ T-LLM ไม่ได้" แปลว่าผู้ดูแลระบบยังไม่ได้ตั้งค่า หรือ gateway ล่ม (ระบบขายไม่กระทบ) วิธีตั้งค่าอยู่ใน `docs/DEPLOY.md` ข้อ 7
